...
class ChatCompletionInput(BaseModel):
    ...
    conversation_id: str | None = Field(
        ...
        description=(
            ...
            "When omitted, the tool always starts a new isolated conversation. "
...
async def do_chat_completion(
    ...
    # Conversation isolation is explicit.  An omitted id always starts a new
    # chat; only a caller-supplied conversation_id may continue an old one.
    ...
    _budgets = (
        DetectorBudgets.from_config(config.chatgpt, validated.model) if config is not None else None
    )
    ...
    return [
        ...
        mcp_types.Tool(
            ...
            description=(
                ...
                "• No conversation_id → new isolated ephemeral chat\n"
                ...
                "Continuation is explicit: retain the conversation_id returned by a response "
                "and pass it on the next call. Omitting it always starts a fresh chat."
            ),
...
def _map_tool_exception(exc: Exception) -> object:
    ...
    if isinstance(exc, OwnedTabRequiredError):
        return mcp_types.CallToolResult(
            content=[
                mcp_types.TextContent(type="text", text=f"{exc}. Retry later. (owned_tab_required)")
            ],
            isError=True,
        )
    ...
    if isinstance(exc, AuthExpiredError):
        return mcp_types.CallToolResult(
            content=[
                mcp_types.TextContent(
                    type="text", text="ChatGPT session expired — re-login required. (auth_expired)"
                )
            ],
            isError=True,
        )
    ...
    async def list_prompts() -> list[mcp_types.Prompt]:
        return [
            ...
            mcp_types.Prompt(
                name="continue-chat",
                description=(
                    "Continue a specific conversation with a follow-up message. "
                    "Requires its conversation id."
                ),
                arguments=[
                    mcp_types.PromptArgument(
                        name="conversation_id",
                        description="Conversation id returned by the previous response",
                        required=True,
                    ),
                    mcp_types.PromptArgument(
                        name="message",
    ...
    async def get_prompt(
        ...
        if name == "ask-chatgpt":
            ...
            if not conversation_id:
                raise ValueError("conversation_id argument is required")
                ...
                messages=[
                    mcp_types.SamplingMessage(
                        ...
                        content=mcp_types.TextContent(
                            type="text",
                            text=(
                                f"Use the chat_completion tool with conversation_id "
                                f"{conversation_id!r} and this follow-up message.\n\n"
                                f"Message: {message}"
                            ),
                        ),
                    ),
