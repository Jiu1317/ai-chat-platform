...
#### Context, reference-image, and asset limits

- A new conversation carries at most 31 complete user-led turns (30 historical
  turns plus the current turn) within a 150,000-character total context budget.
  System instructions have their own 24,000-character cap inside that budget.
- With a valid top-level `conversation_id`, the bridge sends only the current
  user turn and its images because the browser conversation already owns the
  earlier context.
- A request may contain at most 8 reference images. Each original image and all
...
| Multi-turn follow-up | Pass `conversation_id` to resume the exact chat |
| Model listing | <1s |
| Memory listing | <1s |
...
- [x] Image upload via the real CDP file input (PNG/JPEG/WebP/GIF)
...
- [x] Generated image/file links via expiring capability URLs
