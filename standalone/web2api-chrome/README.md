...
`9182` 和 `9183` 各控制一个标签页，所以容量是两路。第三个及之后的请求会等待空闲位置，默认最多等待 60 秒，超时会返回清楚的 `503 capacity_timeout`，避免无期限占用客户端超时；可用环境变量 `W2A_GATEWAY_QUEUE_TIMEOUT` 调整。多开普通 ChatGPT 标签页不会自动提高并发。
