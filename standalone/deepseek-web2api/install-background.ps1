...
$TrayScript = Join-Path $WorkRoot 'deepseek-dual-tab-tray.ps1'
$ExitSignalFile = Join-Path $WorkRoot '.deepseek-tray-exit'
$RunKey = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Run'
$RunName = 'DeepSeekWeb2APITray'
...
if ($Uninstall) {
    ...
    if (Get-ExistingTrayProcesses) {
        Write-Warning 'The tray controller is still closing. Its windows will be restored when it exits.'
    }
    Write-Output 'DeepSeek tray autostart was removed; API and watchdog processes were left unchanged.'
    exit 0
}

if (-not (Test-Path -LiteralPath $TrayScript -PathType Leaf)) {
    throw 'deepseek-dual-tab-tray.ps1 was not found.'
}
$null = [scriptblock]::Create((Get-Content -Raw -LiteralPath $TrayScript))

if ($TrayScript.Contains('"')) { throw 'Paths containing a double quote are not supported.' }
$runCommand = 'powershell.exe -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File "' +
    $TrayScript + '"'
...
Write-Output 'DeepSeek background watchdog and tray controller installed.'
