...
class TestClassifyModel:
    ...
    @pytest.mark.parametrize("slug", [
        ...
        "o3",
        "research",
        "auto",                   # router may select a reasoning model
    ])
    ...
    def test_auto_conservatively_uses_reasoning_budget(self):
        """The web Auto router can silently select a reasoning model, so the
        bridge must not apply the short default first-content timeout."""
