...
def _mapping(*nodes, current_node: str | None = None) -> dict:
    """Build a projected mapping from a list of (id, node_dict)."""
    result = {"nodes": {n[0]: n[1] for n in nodes}}
...
class TestTerminalSelection:
    ...
    def test_tool_preamble_end_turn_is_not_current_terminal_node(self):
        user = _user_node("u-1", "search", 100.0, children=["a-preamble"])
        preamble = _assistant_node(
            "a-preamble", "I'll check that.", 101.0,
            end_turn=True, parent="u-1", children=["tool-1"],
        )
        tool = {
            "id": "tool-1", "parent": "a-preamble", "children": [],
            "message": {
                "id": "tool-1", "author": {"role": "tool"},
                "create_time": 102.0, "end_turn": False,
                "content": {"content_type": "tool_result", "parts": []},
            },
        }
        mapping = _mapping(
            ("u-1", user), ("a-preamble", preamble), ("tool-1", tool),
            current_node="tool-1",
        )
        anchor = TurnAnchor(
            sent_text="search", mode="captured_id",
            captured_user_message_id="u-1",
        )
        ...
        assert text_result.status == "not_ready"
        assert end_result.status == "not_ready"
        assert end_result.diagnostic["reason"] == "text_end_turn_not_current_node"

    def test_final_answer_at_current_node_completes_after_tool_chain(self):
        user = _user_node("u-1", "search", 100.0, children=["a-preamble"])
        preamble = _assistant_node(
            "a-preamble", "I'll check that.", 101.0,
            end_turn=True, parent="u-1", children=["a-final"],
        )
        final = _assistant_node(
            "a-final", "Final sourced answer", 103.0,
            end_turn=True, parent="a-preamble",
        )
        mapping = _mapping(
            ("u-1", user), ("a-preamble", preamble), ("a-final", final),
        ...
        assert text_result.text == "Final sourced answer"
        assert end_result.status == "matched"
        assert end_result.diagnostic["assistant_node"] == "a-final"
