"""Tests for explicit conversation isolation and guarded navigation.

The historical bug: REST/MCP auto-continue trusted ``_current_conv_id``
...
These tests pin:
  ...
  - omitted ids start fresh in REST + MCP; explicit ids still resume
"""
...
# ── 5. REST omitted id starts an isolated chat ────────────────────────


@pytest.mark.asyncio
async def test_rest_omitted_id_starts_new_isolated_chat(monkeypatch):
    """An omitted REST conversation_id never inherits process-local state."""
    ...
    # Fake request: a user message with no conversation_id, despite matching
    ...
    driver.navigate_new_chat.assert_awaited_once_with(gizmo_id=None, model_slug="auto")
    driver.ensure_current_conversation.assert_not_awaited()
    assert driver._current_conv_id is None
    assert server._last_conv_id is None
    assert reached["past_guard"] is True


# ── 6. MCP omitted id starts an isolated chat ─────────────────────────


@pytest.mark.asyncio
async def test_mcp_omitted_id_starts_new_isolated_chat():
    """MCP follows the same explicit-only continuation contract as REST."""
...
@pytest.mark.asyncio
async def test_mcp_explicit_id_still_continues_requested_chat():
    """An explicit id remains the sole supported continuation mechanism."""
    from chatgpt_web2api import mcp_server as mod
    from chatgpt_web2api.config import Config

    driver = MagicMock()
    driver._current_conv_id = "unrelated-local-id"
    driver.ensure_current_conversation = AsyncMock()
    driver.select_model = AsyncMock(return_value=True)
    driver.navigate_new_chat = AsyncMock()
    driver.navigate_conversation = AsyncMock()
    ...
    async def _boom(text, timeout=120, *, budgets=None, model=None):
        raise AssertionError("reached send")
        yield  # pragma: no cover

    driver.send_and_stream = _boom

    with pytest.raises(AssertionError, match="reached send"):
        await mod.do_chat_completion(
            driver,
            {"message": "follow up", "conversation_id": "conv-explicit"},
            Config.load(None),
            on_progress=None,
        )

    driver.navigate_conversation.assert_awaited_once_with("conv-explicit")
    driver.navigate_new_chat.assert_not_awaited()


@pytest.mark.asyncio
async def test_new_chat_clears_conversation_state_before_navigation_failure():
    """A failed Page.navigate cannot leave the prior conversation eligible."""
    driver = CDPDriver(cdp_port=9222)
    driver._current_conv_id = "conv-stale"
    driver._last_response_assets = [{"file_id": "old-file"}]
    driver._cdp = AsyncMock(side_effect=RuntimeError("navigate failed"))

    with pytest.raises(RuntimeError, match="navigate failed"):
        await driver.navigate_new_chat(model_slug="auto")

    assert driver._current_conv_id is None
    assert driver._last_response_assets == []
