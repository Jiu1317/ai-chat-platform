...
Fix 1: after click_send + UUID wait, verify at least one acknowledgment:
  - UUID captured, OR
  - visible generation signal, OR
  - assistant-message DOM count increased, OR
  - user-message DOM count increased AND composer cleared.
  If none, raise before entering completion detection.
...
async def test_send_acknowledged_when_generation_starts_before_dom_updates():
    ...
    result = await driver._verify_send_acknowledged(initial_assistant_count=0)
