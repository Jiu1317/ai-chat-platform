...
def test_browser_asset_limit_is_thirty_mib():
    assert MAX_BROWSER_ASSET_BYTES == 30 * 1024 * 1024
...
@pytest.mark.asyncio
async def test_response_asset_prefers_direct_binary_download():
    client, driver = _make_client()
    with patch(
...
@pytest.mark.asyncio
async def test_browser_asset_download_uses_thirty_mib_guard():
    client, driver = _make_client()
...
@pytest.mark.asyncio
async def test_response_asset_direct_timeout_reaches_browser_fallback():
    client, _ = _make_client()
    ...
    with (
        patch(
            "chatgpt_web2api.backend_client.DIRECT_ASSET_DOWNLOAD_TIMEOUT_SECONDS",
            0.01,
        ),
        patch(
            "chatgpt_web2api.backend_client._download_remote_image",
            new=stalled_download,
        ),
    ):
        result = await client.download_response_asset({
            "source_url": "https://cdn.example/image.png",
