...
class ChatGPTDom:
    ...
    async def upload_files(self, file_paths: list[str]) -> None:
        """Attach local files through ChatGPT's real composer file input."""
        if not file_paths:
            return
        d = self._driver
        if not await d._has_composer():
            from .cdp_driver import SendReadinessError

            raise SendReadinessError("No composer found for file upload")

        expression = (
            "(function(){"
            " var form=document.querySelector('form:has(#prompt-textarea),form:has(.ProseMirror)');"
            " var inputs=Array.from((form||document).querySelectorAll('input[type=file]'));"
            " var el=inputs.find(function(x){return !x.disabled &&"
            "   ((x.accept||'').includes('image') || x.multiple);})"
            "   || inputs.find(function(x){return !x.disabled;}) || null;"
            " return el;"
            "})()"
        )
        evaluated = await d._cdp(
            "Runtime.evaluate",
        ...
        if not object_id:
            # Some layouts create the hidden input only after the + button opens.
            await d._js(
            ...
            object_id = remote.get("objectId")
        if not object_id:
            await d._capture_selector_diagnostic("file input (upload_files)")
            from .cdp_driver import SendReadinessError

            raise SendReadinessError("ChatGPT file input was not found")

        described = await d._cdp("DOM.describeNode", {"objectId": object_id})
        backend_node_id = (
            described.get("result", {}).get("node", {}).get("backendNodeId")
        )
        if not backend_node_id:
            from .cdp_driver import SendReadinessError

            raise SendReadinessError("Could not resolve ChatGPT file input")
        await d._cdp(
            "DOM.setFileInputFiles",
            {"files": file_paths, "backendNodeId": backend_node_id},
            timeout=30,
        )
        expected_count = len(file_paths)
        for _ in range(40):
            state = await d._js(
                "(function(){var form=document.querySelector("
                "'form:has(#prompt-textarea),form:has(.ProseMirror)');"
                "var inputs=Array.from((form||document).querySelectorAll('input[type=file]'));"
                "var count=inputs.reduce(function(n,x){return n+(x.files?x.files.length:0);},0);"
                "var text=(form&&form.innerText)||'';"
                "return JSON.stringify({count:count,error:/upload failed|unsupported file/i.test(text)});"
                "})()"
            )
            try:
            ...
            if parsed.get("error"):
                ...
                raise SendReadinessError("ChatGPT rejected an uploaded image")
            if int(parsed.get("count") or 0) >= expected_count:
                logger.info("Attached %d image(s)", expected_count)
                return
            await asyncio.sleep(0.25)
        from .cdp_driver import SendReadinessError

        raise SendReadinessError("Timed out waiting for ChatGPT image attachments")
