...
def _is_current_terminal_node(
    mapping: dict, node_id: str, node: dict, user_node_id: str,
) -> bool:
    """Return whether a terminal candidate is the conversation branch tip.

    ChatGPT tool turns can mark an assistant preamble (for example, "I'll
    check that") with ``end_turn=true`` before appending web-search/tool nodes
    and the real final answer.  ``current_node`` advances along that chain, so
    an older text node must not complete the API request merely because its
    own flag is terminal.  Older/raw fixtures that do not expose
    ``current_node`` keep the legacy behavior.
    """
    current_node = mapping.get("current_node")
    if not current_node:
        return True
    nodes = mapping.get("nodes") or mapping.get("mapping") or {}
    # ``current_node`` is global to the conversation.  If callers resolve an
    # older historical turn while a newer sibling turn is current, it must not
    # invalidate that older turn.  Apply the branch-tip rule only when the
    # current node actually descends from the submitted user node.
    cursor = current_node
    seen: set[str] = set()
    current_belongs_to_turn = False
    while cursor and cursor not in seen:
        if cursor == user_node_id:
            current_belongs_to_turn = True
            break
        seen.add(cursor)
        cursor_node = nodes.get(cursor)
        if cursor_node is None:
            break
        cursor = cursor_node.get("parent")
    if not current_belongs_to_turn:
        return True
    return current_node in {node_id, _node_id(node)}
...
def select_text_for_turn(mapping: dict, anchor: TurnAnchor) -> TurnTextResult:
    ...
    if text_candidates:
        ...
        if end_turn_text:
            ...
            if not _is_current_terminal_node(mapping, best[0], best[1], user_nid):
                return TurnTextResult("not_ready", diagnostic={
                    "reason": "terminal_text_not_current_node",
                    "user_node": user_nid,
    ...
    if non_text_end_turn and had_non_text_content:
        ...
        if not _is_current_terminal_node(mapping, best[0], best[1], user_nid):
            return TurnEndResult("not_ready", diagnostic={
                ...
                "assistant_node": best[0],
                "current_node": mapping.get("current_node"),
                "reason": "non_text_end_turn_not_current_node",
            })
