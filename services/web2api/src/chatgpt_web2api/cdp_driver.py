...
class ModelSelectionError(ValueError):
    """Raised when a requested ChatGPT Web model is not available to the account."""

    def __init__(self, model: str) -> None:
        self.model = model
        super().__init__(f"Requested ChatGPT Web model is unavailable: {model}")
...
class CDPDriver:
    ...
    def __init__(
        ...
        *,
        ...
        self._last_response_assets: list[dict] = []
    ...
    async def select_model(self, slug: str) -> bool:
        ...
        The live backend model catalog is authoritative.  On the current
        ChatGPT UI, new-chat model choice is applied by ``?model=<slug>`` and
        the old header picker may not exist, so a catalog match is sufficient;
        ``navigate_new_chat`` applies the validated slug immediately after
        this call.  The DOM picker remains a fallback for deployments where
        the catalog cannot be read.
        """
        if slug in ("auto", None, ""):
            return True  # auto is the default, no action needed

        # Prefer the authenticated backend catalog over brittle DOM labels.
        # This also prevents an invented/expired slug from silently running on
        # whichever model happened to be active in the browser.
        try:
            available = {
                str(model.get("slug", ""))
            ...
            logger.debug("Could not validate model against backend catalog: %s", exc)
            available = set()
        if available:
            if slug not in available:
                logger.warning("Model '%s' is not present in the account model catalog", slug)
                return False
            self._current_model = slug
            logger.info("Model validated by account catalog: %s", slug)
            return True
    ...
    async def navigate_new_chat(
        self, gizmo_id: str = None, model_slug: str = "auto"
    ) -> None:
        """Navigate to a fresh chat. Optionally scope to a project gizmo.

        Conversation-bound state is invalidated *before* Page.navigate.  If
        navigation fails, callers must not retain an id or response assets from
        the previous page and accidentally treat them as the new request's.
        """
        self._current_conv_id = None
        ...
        if gizmo_id:
            ...
            safe_model = (
                model_slug if re.fullmatch(r"[A-Za-z0-9._-]+", model_slug or "") else "auto"
            )
            url = f"https://chatgpt.com/?model={safe_model}"
    ...
    async def navigate_conversation(self, conversation_id: str) -> None:
        ...
        is the invariant explicit continuation depends on: ``_current_conv_id``
    ...
    async def upload_files(self, file_paths: list[str]) -> None:
        ...
        await self._dom.upload_files(file_paths)
    ...
    async def _verify_send_acknowledged(
        self, initial_assistant_count: int | None = None
    ) -> bool | None:
        ...
        Acknowledgment signals: a visible generation Stop button, an increased
        assistant-message count, or user-message count increased AND composer
        cleared.
        ...
        Tri-state return:
          - True: acknowledged by any positive signal above
        ...
        Polls for up to 12s at 0.5s intervals. Never raises.
        """
        import time as _time
        ...
        while _time.monotonic() < deadline:
            try:
                result = await self._js_strict(
                    ...
                    "  var generationActive = !!(stopButton && (stopButton.offsetParent !== null || stopButton.getClientRects().length > 0));"
                    "  return JSON.stringify({userCount: userMsgs, assistantCount: assistantMsgs, composerPresent: composerPresent, composerEmpty: composerEmpty, generationActive: generationActive});"
                    "})()"
                )
                ...
                if state.get("generationActive"):
                    return True
                if (
                    initial_assistant_count is not None
                    and int(state.get("assistantCount", 0)) > initial_assistant_count
                ):
                    return True
    ...
    async def send_and_stream(
        ...
        *,
        ...
        if expect_non_text:
            try:
                raw_actions = await self._js_strict(
                    "document.querySelectorAll("
                    "'[data-testid=\"good-image-turn-action-button\"], "
                    "[data-testid=\"bad-image-turn-action-button\"]'"
                    ").length"
                )
                initial_non_text_action_count = int(raw_actions or 0)
            except (CDPJSError, TypeError, ValueError):
                # Best-effort baseline. Fresh chats naturally use zero; on an
                # unreadable existing page the backend anchor remains the guard.
                initial_non_text_action_count = 0
            try:
                raw_asset_ids = await self._js_strict(
                    ...
                    " if(!seen[id]){seen[id]=true;ids.push(id);}}catch(e){}"
                    "});return JSON.stringify(ids);})()"
                )
                decoded_asset_ids = json.loads(raw_asset_ids)
                if isinstance(decoded_asset_ids, list):
                    initial_non_text_asset_ids = tuple(
                        str(item) for item in decoded_asset_ids if item
                    )
            except (CDPJSError, json.JSONDecodeError, TypeError):
                initial_non_text_asset_ids = ()
        ...
        try:
            ...
            #   2. a visible generation signal appeared, OR
            #   3. assistant-message count increased, OR
            #   4. user-message count increased AND composer cleared
            ...
            if conv_id:
                ...
                dom_non_text_assets = list(self._completion.non_text_dom_assets)
                ...
                last_diagnostic = {}
                # When the exact action button inside the current assistant
                # turn completed detection, the DOM text is already terminal.
                # Do not wait another 30 seconds for a backend projection that
                # may stay not_ready when request-identity capture was missed.
                reconcile_attempts = (
                    0
                    if (
                        (
                ...
                for _ in range(reconcile_attempts):
                    ...
                    if result.status == "matched" and result.text:
                        ...
                        if delta:
                            ...
                            last_dom_text += delta
                        elif result.text != last_dom_text:
                            logger.warning(
                                "Final backend text rewrote an already-streamed prefix; "
                                "skipping unsafe suffix to prevent duplicate output "
                                "(streamed=%d final=%d)",
                                len(last_dom_text),
                                len(result.text),
                            )
                    ...
                    elif had_non_text_content and dom_non_text_assets:
                        logger.info(
                            "Using %d completed current-turn DOM image asset(s); "
                            "skipping lagging backend reconciliation",
                            len(dom_non_text_assets),
                        )
                ...
                if had_non_text_content:
                    backend_assets = []
                    try:
                        from .multimodal import extract_response_assets
                ...
                if (
                    not last_dom_text
                    and had_non_text_content
                    and not self._last_response_assets
                ):
                    placeholder = (
    ...
    async def download_response_asset(self, asset: dict) -> dict:
        """Download a model-generated image or file."""
        return await self._backend_client.download_response_asset(asset)
